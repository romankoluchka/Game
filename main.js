...
const game = {
    ...
    generateOptionShip(sizeShip) {
        ...
        if (direction) {
            ...
            y = Math.floor(Math.random() * (10 - sizeShip));
        } else {
            x = Math.floor(Math.random() * (10 - sizeShip));
        ...
        if (this.checkCollision(ship.location)) {
            return this.generateOptionShip(sizeShip)
        }
    ...
    addCollision(location) {
        for (let i = 0; i < location.length; i++) {
            ...
            for (let j = 0; j < startCoordX + 3; j++) {
                const startCoordY = location[i][1] - 1;

                for (let z = 0; z < startCoordY + 3; z++) {
